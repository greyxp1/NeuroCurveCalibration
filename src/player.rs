...
pub fn setup_player(mut commands: Commands) {
    ...
    // The Player component might just hold state like speed if needed.
     commands.insert_resource(Player::default()); // Or add to camera entity if preferred
...
pub fn player_movement(
    ...
    player_settings: Res<Player>, // Access speed from the Player resource/component
) {
    ...
    if let Ok(mut camera_transform) = camera_query.get_single_mut() {
        ...
        if let Some(normalized_direction) = direction.try_normalize() {
             ...
             camera_transform.translation += normalized_direction * player_settings.speed * delta_time;
        }
    }
}